...
app.delete('/delete/:student_id', async function(req, res){
  console.log(req.params.student_id)
  var student = Model.findOne({ID: req.params.student_id});
  if(student === null){
    console.log("Not working");
    res.status(400).send("Error")
  }
  else{
    await Model.findOneAndDelete({ID: req.params.student_id});
    console.log('Deleted')
    res.status(200).send(`Document has been deleted..`);
    }
})
...
//Function that searches and retrieves students by their lastnames
app.get('/search/:last_name' , async function(req, res){{
  console.log("Hi")
  try{
    const data = await Model.find({last_name: req.params.last_name});
    res.status(200).send(data);
  }
  catch(error){
    console.log("error");
